...
const TOME_DEFS = [
  ...
  { id:'bloody',     name:'Bloody Tome',           emoji:'🩸',  color:'#ff4466', desc:'+10% chance to heal 1 HP on hit',    apply: s => { s.bloodHeal = Math.min(1, s.bloodHeal + 0.10); } },
  ...
  if (playerY < 0) {
    ...
    if (wasAirborne) {
      if (_jumpBuffer > 0 && _perfectCooldown <= 0) {
        ...
        // Behind gust: fires on 3rd perfect jump and every one after in succession
